...
function App() {
  const [currentUser, setCurrentUser] = React.useState({ name: '', about: '' }) // Стейт текущего пользователя
  const [cards, setCards] = React.useState([]) // Стейт массива карточек
  const [isEditProfilePopupOpen, setEditProfilePopupOpen] = React.useState(false) // Стейт  открытого попапа редактирования профиля
  ...
  const isAnyPopupOpen = isEditProfilePopupOpen || isAddPlacePopupOpen || isEditAvatarPopupOpen || isImagePopupOpen || isInfoTooltipOpen
  ...
  return (
    <CurrentUserContext.Provider value={currentUser}>
      <div className="page">
        <Header isLoggedIn={isLoggedIn} onLogout={handleLogOut} headerEmail={headerEmail} />
        <Routes>
        ...
        <PopupWithForm class="confirm-deletion" id="confirm-deletion" name="confirm-deletion" title="Вы уверены?">
